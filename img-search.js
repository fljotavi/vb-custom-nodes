...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  // img-search.js
  var ImageSearch = class extends s4 {
    constructor() {
      super();
    }
    render() {
    ...
    async runWithInputs(input) {
      const { keywords, cx, apiKey } = input;
      let imageUrls = "";
      if (keywords && cx && apiKey) {
        try {
          let res = await fetch(`https://www.googleapis.com/customsearch/v1?q=${keywords}&cx=${cx}&key=${apiKey}&searchType=image`);
          let json = await res.json();
          imageUrls = json.items.map((item) => item.image.thumbnailLink).join("\n");
        } catch (e4) {
          console.log("Unable to fetch images", e4);
        }
      }
      this.dispatchEvent(new CustomEvent("outputs", { detail: { imageUrls } }));
    }
  };
  customElements.define("image-search", ImageSearch);
  window.visualblocks.registerNodeSpec({
    "id": "image_search",
    "runnerId": "image-search",
    "name": "Image Search",
