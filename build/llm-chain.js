...
\f\r"'\`<>=]|("|')|))|$)`, "g");
  ...
  var LLMChain = class extends s4 {
    ...
    async runWithInputs(input) {
      ...
      if (prompt && model) {
        this.prompt = prompt;
        this.answer = await model.predict(prompt);
      }
      this.dispatchEvent(new CustomEvent("outputs", { detail: { answer: this.answer } }));
    }
  };
  __publicField(LLMChain, "properties", {
    prompt: {},
    answer: {}
  });
  __publicField(LLMChain, "styles", i`
    h3 {opacity: 0.6;}
  `);
  customElements.define("llm-chain", LLMChain);
  window.visualblocks.registerNodeSpec({
    "id": "llm-chain",
    "runnerId": "llm-chain",
    "name": "LLM Chain",
    "description": "Generates a LLMChain for LangChain.",
    "category": "model",
    "propertySpecs": [],
    "inputSpecs": [
      {
